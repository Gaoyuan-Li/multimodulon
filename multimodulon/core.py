...
class MultiModulon:
    ...
    def _get_attr(self, attr_string: str, attr_id: str, ignore: bool = False) -> Optional[str]:
        """
    ...
    def gff2pandas(self, gff_file: str, feature: str = "CDS", index: Optional[str] = None) -> pd.DataFrame:
        ...
        for gff in gff_file:
            ...
            DF_gene["locus_tag"] = DF_gene.attributes.apply(
                self._get_attr, attr_id="locus_tag", ignore=True
            )
            DF_gene["old_locus_tag"] = DF_gene.attributes.apply(
                self._get_attr, attr_id="old_locus_tag", ignore=True
            )
            ...
            DF_cds["locus_tag"] = DF_cds.attributes.apply(self._get_attr, attr_id="locus_tag")

            DF_cds["gene_name"] = DF_cds.attributes.apply(
                self._get_attr, attr_id="gene", ignore=True
            )

            DF_cds["gene_product"] = DF_cds.attributes.apply(
                self._get_attr, attr_id="product", ignore=True
            )

            DF_cds["ncbi_protein"] = DF_cds.attributes.apply(
                self._get_attr, attr_id="protein_id", ignore=True
            )
    ...
    def create_gene_table(self) -> None:
        ...
        for species_name, species_data in self._species_data.items():
            ...
            try:
                df_annot = self.gff2pandas(str(gff_file), index='locus_tag')
